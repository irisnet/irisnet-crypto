...
function MarshalBinary(object) {
    ...
    let tx = StdTx.create({msgs: [sendMsg], fee: feeMsg, signatures: [signature], memo: memo});
    let txMsgBuf = StdTx.encode(tx).finish();

    //stdTx amion编码前缀[auth/StdTx]
    //let txPreBuf = Buffer.from([240,98,93,238]);
    let txPreBuf = Buffer.from(amino.GetCode("auth/StdTx"));
    //msg amion编码前缀[cosmos-sdk/Send]
    //let msgPreBuf = Buffer.from([42,44,135,250]);
    let msgPreBuf = Buffer.from(amino.GetCode(txMsg.Type()));
